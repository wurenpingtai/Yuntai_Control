...
【返回值】0：写入成功并获得正确返回报文
       ...
       -7：设置等待时间失败
********************************************************************************************************/
int UART::Get_Start_Angle_Bias(unsigned char *Buffer)
{
	memset(Buffer, 0, MAX_NUM);
	int flag = Get_Genting_Posture_Information(Buffer); //获取姿态信息

	switch (flag) {
	case 0 : {
			CharToInt Course, Roll, Pitch;
			Course.Byte[1] = Buffer[2]; Course.Byte[0] = Buffer[3];
			Roll.Byte[1] = Buffer[4]; Roll.Byte[0] = Buffer[5];
			Pitch.Byte[1] = Buffer[6]; Pitch.Byte[0] = Buffer[7];
			Course_Bias = Course.Int / 100.0;
			Roll_Bias = Roll.Int / 100.0;
			Pitch_Bias = Pitch.Int / 100.0;
...
int UART::Get_Genting_Posture_Information_DEC(unsigned char *Buffer)
{
	...
	case 0: {
			...
			printf("Course_Angle:%2f;Roll_Angle:%2f;Pitch_Angle:%2f\r\n",
			       Course.Int / 100.0 - Course_Bias,
			       Roll.Int / 100.0 - Roll_Bias,
			       Pitch.Int / 100.0 - Pitch_Bias);
			return 0;
		}; break;
...
int uart_main(int argc, char *argv[])
{
	...
	mCom->Pantai_Order_Back_In(Buffer);//回中

	for (int i = -180; i < 180; i = i + 10) {
		for (int j = -40; j < 40; j = j + 5) {

			for (int k = -90; k < 90; k++) {
				mCom->Gimbal_Absolute_Angle_Control_DEC(Buffer, i, j, k, 0);
				px4_usleep(100000);
			}
		}
		...
		if (i == 170) {
			px4_usleep(100000);
			mCom->Pantai_Order_Back_In(Buffer);
			break;
		}
	}
